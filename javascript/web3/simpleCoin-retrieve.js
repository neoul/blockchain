...
(async () => {
   console.log(`${senderAddr} retrieves a contract data (no state change)`);

   // load contract schema and target address
   let simpleCoinContract = new web3.eth.Contract(SimpleCoinAbi, contractAddr);

   // execute the function of the contract
   simpleCoinContract.methods.coinBalance(senderAddr).call(
      (err, result) => { console.log("case 1)", result) }
   );
   simpleCoinContract.methods.coinBalance(senderAddr).call(
      {
         from: senderAddr
